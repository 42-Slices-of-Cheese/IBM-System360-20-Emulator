# target macros
TARGET := main.exe # FILL: target filename
MAIN_SRC := main.cpp # FILL: src file which contains `main()`
...
$(TARGET): build-subdirs $(OBJS) find-all-objs
	@echo -e "\t" CXX $(CFLAGS) $(ALL_OBJS) $(MAIN_SRC) -o $@
	@$(CXX) $(CXXFLAGS) $(ALL_OBJS) $(MAIN_SRC) -o $@
